...
struct currency_core {
 ...
 // uint64_t GENESIS_BLOCK_REWARD;
  ...
  uint64_t MINED_COINS_UNLOCK_WINDOW;
  uint64_t BLOCK_GRANTED_FULL_REWARD_ZONE;
  ...
  void serialize(ISerializer &s) {
    ...
    KV_MEMBER(DECIMAL_POINT)
    KV_MEMBER(MONEY_SUPPLY)
 //   KV_MEMBER(GENESIS_BLOCK_REWARD)
    KV_MEMBER(DEFAULT_DUST_THRESHOLD)
    KV_MEMBER(MINIMUM_FEE)
    KV_MEMBER(MINED_COINS_UNLOCK_WINDOW)
    KV_MEMBER(BLOCK_GRANTED_FULL_REWARD_ZONE)
    KV_MEMBER(PUBLIC_ADDRESS_BASE58_PREFIX)
