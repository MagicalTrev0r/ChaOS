...
void cn_cache_slow_hash_v0(cn_context &context, const void *data, size_t length, Hash &hash)
{
  if(hw_check_aes())
    cryptonight_hash<false, CRYPTONIGHT_CACHE_HASH>(data, length, reinterpret_cast<char *>(&hash), context);
  else
    cryptonight_hash<true, CRYPTONIGHT_CACHE_HASH>(data, length, reinterpret_cast<char *>(&hash), context);
}
