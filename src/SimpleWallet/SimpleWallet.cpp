...
bool simple_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  /* 1000  = 0.001 $CXCHE */
  if (cmd.fee == 1000) {
    feeString = "0.001 $CXCHE (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " $CXCHE";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " $CXCHE, with a fee of " << feeString << std::endl
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      ...
      std::cout << "Cache Wallet v" << PROJECT_VERSION << std::endl;
      std::cout << "Usage: cache-wallet [--wallet-file=<file>|--generate-new-wallet=<file>] [--daemon-address=<host>:<port>] [<COMMAND>]";
  ...
  logger(INFO, BRIGHT_GREEN) << "Cache Wallet v" << PROJECT_VERSION;
