...
{
  TransferCommand::TransferCommand(const CryptoNote::Currency& currency) :
      m_currency(currency), fake_outs_count(0), fee(currency.minimumFee()) {
  }

  bool TransferCommand::parseTransfer(LoggerRef& logger, const std::vector<std::string> &args) {

    CryptoNote::ArgumentReader<std::vector<std::string>::const_iterator> ar(args.begin(), args.end());
  ...
  bool TransferCommand::parseCreateDeposit(LoggerRef& logger, const std::vector<std::string> &args) {
  ...
  bool TransferCommand::parseWithdrawDeposit(LoggerRef& logger, const std::vector<std::string> &args) {
    CryptoNote::ArgumentReader<std::vector<std::string>::const_iterator> ar(args.begin(), args.end());
